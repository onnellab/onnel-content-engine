...
export const Short: React.FC<VideoProps> = (props) => {
  ...
  const step = Math.max(0, props.captions.findIndex((c) => c === caption));
  ...
  return <AbsoluteFill style={{backgroundColor: problem ? '#FFFCF5' : '#FFFFFF', color: ink, fontFamily}}>
    ...
    <AbsoluteFill style={{padding: '78px 72px 110px', opacity: fade}}>
      {problem ? <div style={{display: 'flex', gap: 12, height: 62}}>
        {(props.locale === 'ko' ? ['문제', '해결 단계', '직접 해 보기'] : ['PROBLEM', 'SOLUTION', 'TRY IT']).map((text, i) => <div key={text} style={{...label, padding: '14px 22px', borderRadius: 22, background: i === (closing ? 2 : problemPhase ? 0 : 1) ? '#FFE8DC' : '#F1EDF6'}}>{text}</div>)}
      </div> : <div style={{...label, alignSelf: 'flex-start', padding: '14px 24px', borderRadius: 22, background: '#EEE7FF'}}>{phase} · {step + 1}/{props.captions.length}</div>}
      <div style={{fontSize: 58, lineHeight: 1.22, fontWeight: 760, whiteSpace: 'pre', marginTop: 28, height: 156, display: 'flex', alignItems: 'center'}}><CaptionText text={props.hook} width={924} size={58} weight={760} /></div>
      ...
      <div style={{marginTop: 26, minHeight: 170, borderRadius: 26, padding: '20px 22px', background: problem ? '#FFE8DC' : '#EEE7FF', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
        <div style={{fontSize: 46, lineHeight: 1.32, fontWeight: 700, textAlign: 'center', whiteSpace: 'pre', opacity: captionFade}}>{caption && <CaptionText text={caption.text} width={880} size={46} weight={700} />}</div>
      </div>
      <div style={{fontSize: 36, lineHeight: 1.3, fontWeight: 600, whiteSpace: 'pre', textAlign: 'center', marginTop: 30, opacity: closing ? 1 : 0}}><CaptionText text={props.cta} width={924} size={42} weight={600} /></div>
