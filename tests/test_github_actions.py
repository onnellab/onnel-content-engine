...
class GitHubActionsTest(unittest.TestCase):
    ...
    def test_workflow_documents_required_stages_and_dry_run(self) -> None:
        ...
        self.assertIn("Wait For Pages Deployment", workflow)
        self.assertIn("Commit Content Engine Outputs", workflow)
        self.assertIn("Post Due Core Distribution", workflow)
        self.assertLess(workflow.index("- name: Deploy\n"), workflow.index("- name: Wait For Pages Deployment"))
        self.assertLess(workflow.index("- name: Wait For Pages Deployment"), workflow.index("- name: Commit Content Engine Outputs"))
        self.assertLess(workflow.index("- name: Commit Content Engine Outputs"), workflow.index("- name: Post Due Core Distribution"))
