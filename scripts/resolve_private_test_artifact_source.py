...
def main() -> int:
 ...
 if r['platform']=='ios': out='source=codemagic\n'
  ...
  try: expires=datetime.fromisoformat(q.get('artifact_expires_at',''))
  except ValueError: raise SystemExit('Android AAB expiration metadata is missing or invalid')
  if expires.tzinfo is None or expires<=datetime.now(timezone.utc): raise SystemExit('Android AAB artifact has expired; build a new private-test AAB')
