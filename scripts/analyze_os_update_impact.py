...
def main() -> int:
    ...
    for source in changed:
        ...
        for app in apps:
            app_platforms = set(filter(None, app.get("platforms", "").split("|")))
            app_slug = app.get("slug") or app.get("app_slug", "")
            if platform in app_platforms and app_slug:
                tasks.append({"task_id":f"os-{platform}-{app_slug}","app_slug":app_slug,"platform":platform,"status":"review_required","evidence":{"url":source.get("url"),"content_hash":source.get("content_hash"),"checked_at":source.get("checked_at")},"scope":["OS/API deprecation","permission and file-provider behavior","plugin compatibility","foreground/background lifecycle"],"conclusion":"No compatibility claim made; inspect the official changed release note and app code before action."})
