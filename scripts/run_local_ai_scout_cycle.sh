...
allowed='^(data/store_review_ai_drafts\.json|data/ai_doctor_findings\.json|data/ai_doctor_diagnoses/[^/]+\.json|data/ai_coder_tasks\.json|data/ai_manager_daily_report\.json|data/chatgpt_monitor_snapshot\.json|generated/review-replies/review_packet\.(json|md))$'
changed="$({ git diff --name-only; git ls-files --others --exclude-standard; } | sort -u)"
if [[ -n "$changed" ]] && ! printf '%s\n' "$changed" | rg -v "$allowed" >/dev/null; then
  git add data/store_review_ai_drafts.json data/ai_doctor_findings.json data/ai_coder_tasks.json data/ai_manager_daily_report.json data/chatgpt_monitor_snapshot.json generated/review-replies/review_packet.json generated/review-replies/review_packet.md
...
GITHUB_REPOSITORY=onnellab/onnel-content-engine python3 scripts/dispatch_ai_fix_tasks.py --execute
echo "Local AI Scout cycle completed; only contract-ready GREEN ai-fix labels may dispatch approval."
