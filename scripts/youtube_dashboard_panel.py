...
def youtube_settings_panel():
    ...
    <section class="yt-settings" id="youtube-settings" aria-label="YouTube connection settings">
      ...
      <div><span class="yt-tag">Google Cloud</span><span class="yt-tag">Official API</span><span class="yt-tag"><span class="yt-ko">화·목·토 전체 자동화</span><span class="yt-en">Tue · Thu · Sat full pipeline</span></span></div>
      <p><span class="yt-ko">Aether Inn 신곡은 Lyria 3 Pro로 만들고, Gemini 2.5 Flash Image로 풍경 커버를 생성한 뒤 고정 타이포를 합성해 영상 렌더와 Aether Inn YouTube 업로드까지 이어져요. 공개 대시보드에는 Google 인증정보를 입력하지 않고, 작업용 Mac의 ADC를 사용합니다.</span><span class="yt-en">Aether Inn singles use Lyria 3 Pro, Gemini 2.5 Flash Image for the landscape cover, branded cover compositing, video rendering, and the Aether Inn YouTube uploader. Google credentials are never entered on this public dashboard; the worker Mac uses Application Default Credentials.</span></p>
      <div class="yt-links">
