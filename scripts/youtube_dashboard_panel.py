...
def youtube_settings_panel():
    ...
    <section class="yt-settings" id="youtube-settings" aria-label="YouTube connection settings">
      ...
      <div><span class="yt-tag">Google Cloud</span><span class="yt-tag">Official API</span><span class="yt-tag"><span class="yt-ko">화·목·토 자동 생성</span><span class="yt-en">Tue · Thu · Sat generation</span></span></div>
      <p><span class="yt-ko">Aether Inn 신곡은 Google Lyria 3 Pro로 생성해요. 공개 대시보드에는 Google 인증정보를 입력하지 않고, 버튼을 누르면 작업용 Mac의 로컬 설정창에서 Project ID·후보 수·실행당 비용 한도·자동 생성 허용 여부만 저장해요. Google 인증은 Mac의 Application Default Credentials(ADC)를 사용합니다.</span><span class="yt-en">Aether Inn new singles use Google Lyria 3 Pro. Google credentials are never entered on this public dashboard. The button opens a local Mac console that stores only Project ID, candidate count, per-run spend cap, and generation enablement. Authentication uses Application Default Credentials on the Mac.</span></p>
      <div class="yt-links">
      ...
      <p class="yt-note"><span class="yt-ko">모델은 <code>lyria-3-pro-preview</code>, 위치는 <code>global</code>로 고정해요. 현재 공시 단가는 전체 곡 1회당 US$0.08이며, 로컬 설정의 실행당 비용 한도를 넘는 요청은 차단합니다. 연결 확인은 유료 음악 생성을 실행하지 않아요.</span><span class="yt-en">The model is fixed to <code>lyria-3-pro-preview</code> in <code>global</code>. Current published pricing is US$0.08 per full-song generation; requests above the locally configured per-run cap are blocked. Connection checks do not trigger paid music generation.</span></p>
