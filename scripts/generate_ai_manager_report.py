...
def main() -> int:
    ...
    active_reviews = [item for item in items if item.get("operational_status") != "dismissed"]
    high = [item for item in active_reviews if item.get("category") in {"bug", "data_loss", "security"}]
    pricing = [item for item in active_reviews if item.get("category") == "pricing_confusion" and item.get("similar_reviews", 0) >= 3]
