...
def main() -> int:
    ...
    report = {
        ...
        "summary": {"reviews_triaged": len(items), "high_risk_reports": len(high), "pricing_patterns": len(pricing), "replies_queued": sum(item.get("status") == "queued" for item in queued), "replies_published": sum(item.get("status") == "published" for item in queued), "issues_created": len(issue_items), "new_crash_incidents": sum(row.get("status") == "new" for row in crash_rows), "doctor_high_findings": sum(item.get("severity") in {"high", "critical"} for item in doctor.get("findings", [])), "coder_tasks_proposed": sum(item.get("status") == "proposed" for item in coder_tasks), "coder_draft_prs":sum(item.get("status") == "draft_pr_created" for item in coder_tasks), "policy_assessments_pending":sum(item.get("status") == "approved_for_assessment" for item in policy_tasks), "policy_assessment_failures":sum(item.get("status") == "FAIL" for item in policy_assessments.get("assessments", [])), "release_candidates_failed":len(failed_candidates), "ios_device_qa_blocked":len(stopped_ios), "os_updates_to_review": sum(item.get("status") == "changed" for item in os_updates.get("sources", [])), "os_impact_tasks":len(os_impacts.get("tasks", [])), "store_policy_tasks":len(policy_tasks), "store_submissions_blocked":sum(item.get("status") == "blocked" for item in submissions.get("records", [])), "internal_test_uploads":sum(item.get("status") == "uploaded" for item in internal_submissions.get("submissions", [])), "new_internal_test_feedback":sum(item.get("status") == "new" for item in internal_feedback.get("feedback", [])), "internal_test_high_findings":sum(item.get("severity") in {"high", "critical"} for item in internal_findings.get("findings", [])), "internal_test_passed":sum(item.get("status") == "passed" for item in internal_results.get("results", [])), "internal_test_failed":sum(item.get("status") == "failed" for item in internal_results.get("results", [])), "internal_test_ready_for_upload":sum(item.get("status") == "ready_for_internal_upload" for item in internal_readiness.get("records", []))},
