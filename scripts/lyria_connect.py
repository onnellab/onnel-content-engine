#!/usr/bin/env python3
"""Loopback-only Lyria 3 Pro setup console. Stores non-secret limits only."""
...
from lyria_config import CONFIG_PATH, connection_status, delete_settings, save_settings

ROOT = Path(__file__).resolve().parents[1]
TEMPLATE = ROOT / "templates" / "lyria_connect.html"
PRIVATE_ROOT = Path.home() / "Library/Application Support/ONNELLAB/content-engine/lyria"
SERVICE = "onnellab-lyria-connect-v1"
...
class SetupHandler(BaseHTTPRequestHandler):
    ...
    def reply(self, code, payload=None, *, html=None):
        ...
        self.send_header(
            ...
            "navigate-to https://console.cloud.google.com https://cloud.google.com",
        )
    ...
    def do_GET(self):
        ...
        if path.path == "/api/status" and not path.query:
            ...
            return self.reply(200, connection_status(check_auth=True))
    ...
    def do_POST(self):
        ...
        try:
            ...
            if self.path == "/api/reset":
                ...
                delete_settings(CONFIG_PATH)
                return self.reply(200, connection_status(check_auth=False))
...
def open_console(*, open_browser=True, ttl=7200, private_root=PRIVATE_ROOT):
    ...
    descriptor = os.open(root / "lyria-connect.lock", os.O_CREAT | os.O_RDWR | os.O_NOFOLLOW, 0o600)
    ...
    try:
        try:
            ...
            session = root / "lyria-connect-session.json"
        ...
        if server:
            server.server_close()
            (root / "lyria-connect-session.json").unlink(missing_ok=True)
        os.close(descriptor)
