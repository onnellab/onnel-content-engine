...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    #app-status-grid {{ grid-template-columns: repeat(auto-fill, minmax(min(260px, 100%), 1fr)); }}
    #app-status-grid > .app-status-card {{ min-width: 0; }}
    #app-status-grid > .app-status-card > summary {{ display: grid; grid-template-columns: minmax(0, 1fr); gap: 6px; }}
    #app-status-grid > .app-status-card > summary strong a {{ max-width: 100%; overflow-wrap: anywhere; }}
    #app-status-grid > .app-status-card > summary > span {{ min-width: 0; text-align: left; }}
    .status-card span,
    .release-card span,
