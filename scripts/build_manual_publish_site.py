...
<head>
  ...
  <style>
    ...
    * {{ box-sizing: border-box; }}
    ...
    .platform-status-summary {{ color: var(--muted); font-size: 12px; font-weight: 700; }}
...
<body>
  ...
  <script>
    ...
    function renderPlatformSummary() {{
      ...
      const platformSummaryParts = [`${{t('blogPlatformName')}} ${{blogItems.length}}`];
      ...
      platforms.forEach((label) => {{
        ...
        platformSummaryParts.push(`${{label}} ${{rows.length}}`);
      ...
      platformStatusSummary.textContent = platformSummaryParts.length ? platformSummaryParts.join(' / ') : '';
    }}
    ...
    function card(item) {{
      ...
      [
        [item.language, ''],
        [isDone(item) ? t('doneTag') : item.status, isDone(item) ? 'status-done' : statusClass(item.status)],
