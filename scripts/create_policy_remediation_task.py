...
def main() -> int:
 ...
 ticket={"observed_symptom":f"Verified store policy assessment failed: {scope}","reproduction":f"Re-run the recorded assessment for {a.task_id}.","expected_result":"The approved app paths satisfy the cited store requirement without changing restricted behavior.","allowed_paths":a.allowed_path,"prohibited_paths":["billing","authentication","authorization","privacy","cryptography","database migrations","release signing","store metadata","secrets"],"verification_commands":["flutter analyze","flutter test"],"performance_baseline":"Existing app quality/performance gate must not regress.","completion_criteria":"The recorded policy failure is resolved within the approved paths and every repository quality gate passes."}
 tasks.append({"task_id":new_id,"origin":"store_policy_assessment","policy_task_id":a.task_id,"app_slug":source["app_slug"],"repository":repos.get(source["app_slug"],""),"status":"proposed","risk_class":"YELLOW","intake_status":"ready","ticket":ticket,"escalated_by":a.approver,"escalated_at":datetime.now(timezone.utc).replace(microsecond=0).isoformat(),"approved_scope":scope,"allowed_paths":a.allowed_path,"finding":{"assessment":assessment,"policy_evidence":source.get("evidence",{})},"constraints":["Create a draft PR only; never merge, submit, or deploy.","Change only approved_scope and allowed_paths after reproducing or testing.","Do not modify billing, authentication, authorization, privacy, cryptography, database migrations, signing, store metadata, or secrets.","If the assessment concerns a restricted area, stop and report; do not patch."]})
