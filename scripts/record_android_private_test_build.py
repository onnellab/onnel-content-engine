...
def main() -> int:
 ...
 if not isinstance(rows,list) or any(x.get('release_id')==a.release_id and x.get('status')!='retry_superseded' for x in rows): raise SystemExit('private test build request is invalid or already active')
 now=datetime.now(timezone.utc).replace(microsecond=0)
 rows.append({'task_id':a.task_id,'release_id':a.release_id,'tag':a.tag,'github_run_id':a.run_id,'artifact_name':a.artifact_name if a.status=='artifact_ready' else '','source':'github_actions','status':a.status,'approved_by':a.approver,'dispatched_at':now.isoformat(),'artifact_expires_at':(now+timedelta(days=14)).isoformat() if a.status=='artifact_ready' else ''})
