...
codex exec -s read-only -C "$app_path" -o "$result" "Read '$packet_path' and '$engine_root/prompts/codex_policy_assessment.md'. Do not edit any file. Return only JSON: {task_id, status, evidence:[{reference, detail}], conclusion, patch_authorized:false}. status must be PASS, FAIL, or STOP."
